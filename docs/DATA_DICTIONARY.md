...
`output/final_analytical_dataset.csv` — **1,106 rows × 93 columns**, one row per
...
| `authors_truncated` | logical | Whether the raw ScienceDirect author string was visibly elided | str_detect(authors_raw, '\\.{2,}') — TRUE when the listing ends in an ellipsis | R/02_clean_abstracts.R | TRUE/FALSE | never missing (FALSE when the string was empty) | 1106 | 100.0 | diagnostic | Distinguishes a censored author list from a genuinely short one. It is NOT the same signal as the display cap: a flagged row parses to author_count 4, because the ellipsis consumes a slot, while a capped row shows 5 and is not flagged. Counting only author_count >= 5 therefore understates how many listings are incomplete. Computed at 02 and carried through R/05_adjudicate.R deliberately; it had been dropped from that select once, which made author censoring invisible downstream. Pinned by config/data_contract.yml. |
...
| `is_academic` | logical | Academic-centre affiliation detected | str_detect for university\|academic\|teaching hospital\|school of medicine\|tertiary.*center\|residency\|fellowship | R/02_clean_abstracts.R | TRUE/FALSE | never missing (FALSE when no match) | 1092 | 98.7 | predictor | affiliation_raw is NA at the time this runs, so it is text-only. 0-4% in 2012-2018 vs 22-47% later. |
| `is_us_based` | logical | US location detected | str_detect against state names/abbreviations and a list of major US cities and institutions | R/02_clean_abstracts.R | TRUE/FALSE | never missing (FALSE when no match) | 1092 | 98.7 | predictor | 31-45% in 2012-2018 vs 97-100% in 2019-2023; a text-availability artefact, not a real trend. |
...
| `is_multicenter` | logical | Whether the abstract DESCRIBES the study as multicentre | str_detect on abstract text for multi-?center\|multi-?site\|multi-?institutional\|N center(s)/site(s)/institution(s)/hospital(s) | R/02_clean_abstracts.R | TRUE/FALSE | never missing (FALSE when no match) | 1106 | 100.0 | predictor (Aim 2 and Aim 3) | A CLAIM MADE IN THE TEXT, not a verified design fact. TRUE for 65 of 1,106. It is not the same as the authors coming from several institutions: 679 abstracts name more than one affiliation, and 58 of the 65 do both, so the two agree where the text is explicit and diverge where it is silent. A two-site study that never uses the word is counted FALSE, so this under-counts multicentre conduct. PI decision, 2026-09-05: retained as the design claim rather than redefined as n_affiliations > 1, which would measure collaboration breadth instead. See appendix A22. |
...
| `first_author_country` | character | Country of the first author | parse_country() in R/utils_states.R | R/09c_author_characteristics.R | Australia \| Brazil \| Canada \| China \| Czech Republic \| Denmark \| France \| India \| Israel \| Italy \| Japan \| Netherlands \| New Zealand \| South Africa \| South Korea \| Spain \| Sweden \| Switzerland \| Taiwan \| Turkey \| UAE \| United Kingdom \| USA | no country parsed | 197 | 17.8 | descriptive |  |
...
| `gender_unified` | character | Inferred gender of the first author | coalesce() in the fixed order nppes > npi > openalex > pubmed_fullname > obgyn_pubs > openalex_search > orcid > open_payments > senior_triangulation > second_triangulation > ssa | R/10e_merge_demographics.R | female \| male | no source returned a gender | 1082 | 97.8 | predictor (Aim 2 and Aim 3) | Tiers 1-2 are registry lookups (267 abstracts); tiers 3-10 infer from a full given name; tier 11 from a single initial (287). Not self-reported. |
| `gender_source` | character | Which tier supplied gender_unified | First non-NA tier in the same order | R/10e_merge_demographics.R | npi \| nppes \| obgyn_pubs \| open_payments \| openalex \| openalex_search \| orcid \| pubmed_fullname \| senior_triangulation \| ssa | gender_unified is NA | 1082 | 97.8 | provenance | Read this before using gender_unified: 'nppes' and 'npi' are registry-reported, every other value is name-inferred, and 'ssa' rests on a single initial. |
| `gender_n_sources` | numeric | Number of gender sources that returned any value | rowSums(!is.na(across(gender columns))) | R/10e_merge_demographics.R | [0, 6] | never missing | 1106 | 100.0 | QC |  |
