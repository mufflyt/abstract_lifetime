...
## test-cycle20_gender_resolution.R

### 18. gender_conflict marks exactly the abstracts in the conflict log

**What fails.** In committed state, data/processed/gender_conflicts.csv holds 231 rows while final_analytical_dataset.csv flags 282 abstracts as gender_conflict. Every logged abstract is flagged; 51 flagged abstracts are absent from the log. R/10e_merge_demographics.R writes both in the same run, so the pair cannot disagree unless one was committed from a different run: the dataset was regenerated by the gender-correction refit and the log was not committed with it. The test passes on a machine holding the regenerated log, which is why it was caught only by running the gate in a clean worktree.

**Decision needed.** None scientific. This is operational: re-run R/10e_merge_demographics.R and commit gender_conflicts.csv together with the dataset it describes. It is registered rather than fixed here only because the regenerated file is currently an uncommitted change belonging to a concurrent agent in the shared working tree, and committing another agent's in-flight work would be worse than recording the gap. Remove this entry as soon as that lands.

**Documented in.** tests/loop/LEDGER.md cycle 23
...
**Decision needed.** Regenerating means re-running the whole search layer, which changes candidate sets and invalidates the human adjudication recorded against the current candidates. Same decision as the cycle 4 entry, and it should be taken with it rather than separately.
...
**Decision needed.** Whether the remaining 156 are reviewed, or whether the study reports that the mid-score band was resolved by algorithm for the majority of the cases flagged for human eyes. This is a completeness statement about the adjudication and belongs in the methods either way.
...
**Decision needed.** Whether fidelity checking should extend to the human-adjudicated matches. Running it there is cheap and changes no estimand by itself, but the result may show that some accepted matches are a different piece of work from the abstract, which would then be an adjudication question. Extending the check is engineering; acting on what it finds is not.
