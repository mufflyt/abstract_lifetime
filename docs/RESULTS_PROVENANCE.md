...
| RCT → time to publication | HR **2.146** (1.432–3.214), p < 0.001 | `output/aim2b_cox_regression.csv` | `coxph` | ✅ 170 events, was 104. Proportional hazards holds for this term (p = 0.13) |
| RCT → publication | OR **2.556** (1.551–4.156), p < 0.001 | `output/aim3_logistic_regression.csv` | `glm` | ✅ n = 1,010 |
| Multicenter → time to publication | HR **1.372** (0.800–2.353), p = 0.250 | `output/aim2b_cox_regression.csv` | | ✅ not significant; the README claim of HR ≈ 2.3 has been removed |
...
| Author count → time to publication | HR **1.266** (1.101–1.455), p = 0.001 | `output/aim2b_cox_regression.csv` | | ⚠️ **an average over follow-up, not a constant.** This term violates proportional hazards (p = 0.002); do not quote it alone. `n_authors` is also censored at 5 |
| Author count, 12 months | HR **1.331** (1.146–1.546) | `output/cox_time_varying_hr.csv` | `coxph(+ tt())` | ✅ time-varying fit, resolved 2026-09-04 |
| Author count, 24 months | HR **1.528** (1.258–1.857) | same | | ✅ |
| Author count, 3 months | HR **1.010** (0.839–1.216) | same | | ✅ no detectable early effect — this is what the constant 1.257 was averaging |
| Cox PH global test | p = **0.041**, violated; **0.71** after stratifying on `n_authors` | `output/cox_ph_assumption.csv`, `output/cox_ph_terms.csv` | `cox.zph` | ✅ diagnosed per term and remediated |
| Author count → publication | OR **1.336** (1.154–1.560), p < 0.001 | `output/aim3_logistic_regression.csv` | | ⚠️ same |
| Male first author → time to publication | HR **0.779** (0.573–1.059), p = 0.110 | `output/aim2b_cox_regression.csv` | | ⚠️ not significant. Moved again on 2026-09-04 when NPPES registry gender became tier 1; 231 abstracts carry a cross-source conflict |
| US-based → time to publication | HR **1.374** (0.860–2.195), p = 0.184 | same | | ⚠️ **no longer significant** after `is_us_based` was re-derived from the backfilled text (689 → 907 TRUE) |
