...
gender_col <- if ("gender_unified" %in% names(d)) {
  "gender_unified"
} else if ("first_author_gender" %in% names(d)) {
  "first_author_gender"
} else {
  NULL
}
missing <- if (!is.null(gender_col)) {
  d |> filter(is.na(.data[[gender_col]]))
} else {
  d  # no gender column yet — process all
}
missing_abs <- abs |>
